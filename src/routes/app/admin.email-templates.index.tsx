 ...
 *
 * Split from the parent route (which became an Outlet shim) so sibling
 * routes like /admin/email-templates/new and /admin/email-templates/:id
 * mount alongside instead of stacking inside.
 */
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-semibold tracking-tight text-ink">
              {t("admin.emailTemplates.title", { defaultValue: "Message templates" })}
            </h1>
          </div>
          <p className="mt-1 text-sm text-ink-muted">
            {t("admin.emailTemplates.subtitle", {
              defaultValue:
                "Manage notification templates across all channels — email, in-app, Teams, Slack.",
            })}
          </p>
        </div>
        <Link
          to="/app/admin/email-templates/new"
          className="inline-flex items-center gap-1.5 rounded-md bg-gold px-3 py-2 text-sm font-medium text-ink hover:bg-gold/90"
        >
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {templates.map((tpl) => (
                  ...
                  >
                    <td className="px-4 py-3 font-mono text-xs text-ink">{tpl.templateId}</td>
                    ...
                    <td className="max-w-xs px-4 py-3 text-ink-muted">
                      <span className="line-clamp-1">{tpl.subjectEn ?? "—"}</span>
                    </td>
